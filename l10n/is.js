OC.L10N.register(
    "twofactor_gateway",
    {
    "%s is your authentication code" : "%s er auðkenningarkóðinn þinn á Nextcloud",
    "Signal verification" : "Signal sannvottun",
    "Authenticate via Signal" : "Auðkenning með Signal",
    "Message gateway verification" : "Gátt fyrir auðkenningu með skilaboðum",
    "Authenticate via SMS" : "Auðkenning með SMS",
    "Telegram verification" : "Telegram sannvottun",
    "Authenticate via Telegram" : "Auðkenning með Telegram",
    "Two-Factor Gateway" : "Tveggja-þrepa auðkenningargátt",
    "Two-factor gateway providers (SMS, Telegram, Signal)" : "Útgefandi tveggja-þrepa auðkenningargáttar (SMS, Telegram, Signal)",
    "A set of Nextcloud two-factor providers to send authentication codes via Signal, SMS and Telegram." : "Nokkrit útgefendur tveggja-þrepa auðkenningar til að senda auðkenningarkóða með SMS, Telegram og Signal.",
    "Authentication code" : "Auðkenningarkóði",
    "An access code has been sent to %s" : "Búið er að senda aðgangskóða á %s",
    "Error while sending the Message. Please try again later or contact the administrator." : "Villa við að senda skilaboðin. Reyndu aftur seinna eða hafðu samband við kerfisstjóra.",
    "Your phone number has changed and had not been re-verified. Use a different two-factor mechanism or contact your admin." : "Símanúmerið þitt breyttist og er ekki búið að sannreyna það aftur. Notaðu aðra tveggja-þrepa auðkenningaraðferð eða hafðu samband við kerfisstjórann þinn."
},
"nplurals=2; plural=(n % 10 != 1 || n % 100 == 11);");
