...
class SessionHealthService {
	...
	private function handleCriticalLoggedOut(): void {
		$alreadyMarked = ReconfigurationState::isRequired($this->appConfig);
		ReconfigurationState::markRequired($this->appConfig);

		try {
			$this->jobList->remove(GoWhatsAppSessionMonitorJob::class, null);
		} catch (\Throwable $e) {
			$this->logger->warning('Failed to stop GoWhatsApp monitor after logged_out state.', [
				'exception' => $e,
			]);
		}

		if ($alreadyMarked) {
			$this->logger->debug('GoWhatsApp logged_out state already handled; monitor remains disabled until reconfiguration.');
			return;
		}

		$this->logger->warning('GoWhatsApp session health: device is logged_out → dispatching CRITICAL event and disabling monitor until reconfiguration.');
